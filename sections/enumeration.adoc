...
<2> Here we define a <<type-alias,Type Alias>> for Enumerations internal `Value` type, since we make the name match
...
<4> This import causes two things, first we can refer to `Mon` without prefixing it with `WeekDay`, but it also brings the `type WeekDay` into scope, so we can use it in the method definition below
